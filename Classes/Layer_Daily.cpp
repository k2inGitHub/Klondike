...
void Layer_Daily::updateLayout(){
    ...
    horizonAliament(_labels, 26, VisibleRect::top().y - ht - 13);
...
//            float w = l->getContentSize().width;
    ...
    
    {
        ...
        auto topbg = ImageView::create("_rili-1.png");
        topbg->setScale9Enabled(true);
        topbg->setCapInsets(Rect(0, 0, 38, 127));

        CCLOG("getCapInsets = %s", CCStringFromRect(topbg->getCapInsets())->getCString());
        topbg->setContentSize(Size(590, 102));
        topbg->setPosition(VisibleRect::top() + Vec2(0, -135));
        addChild(topbg, -2);
        _red1 = topbg;
        
        auto bg = ImageView::create("_rili-2.png");
        bg->setPosition(VisibleRect::top() + Vec2(0, -420));
        bg->setScale9Enabled(true);
        bg->setContentSize(Size(590, 470));
        addChild(bg , -3);
        _white1 = bg;
    }
    //21
